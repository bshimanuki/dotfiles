...
alias gt='git status'
glgl(){git log --topo-order --graph --pretty=format:"${_git_log_oneline_format}" $(git show-ref $(git for-each-ref --format='%(refname:short) %(upstream:short)' refs/heads) | cut -d' ' -f2)}
